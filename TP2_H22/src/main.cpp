...
static TrouDeVer *trouDeVer = NULL;
...
void calculerPhysique( )
{
    ...
    {
        // static int sensX = -1;
        // Etat::whDim.x += sensX * 0.01;
        // if ( Etat::whDim.x < 10.0 ) sensX = +1;
        // else if ( Etat::whDim.x > 13.0 ) sensX = -1;

        // static int sensZ = 1;
        // Etat::whDim.z += sensZ * 0.01;
        // if ( Etat::whDim.z < 10.0 ) sensZ = +1;
        // else if ( Etat::whDim.z > 13.0 ) sensZ = -1;
...
{
    // donner la couleur de fond (l'espace est pas mal noir)
    ...
    // créer le trou de ver
    trouDeVer = new TrouDeVer();
}
...
void FenetreTP::afficherScene( )
{
    ...
    matrProj.Perspective( 50.0, (GLdouble) largeur_ / (GLdouble) hauteur_, 0.1, 150.0 );
    glUniformMatrix4fv( locmatrProjBase, 1, GL_FALSE, matrProj );

    // ... la matrice de visualisation (point de vue)
    if ( Etat::exoplaneteChoisie > 0 )
        trouDeVer->positionnerCamera( Etat::exoplaneteChoisie );
    else
        camera.definir();
...
void FenetreTP::clavier( TP_touche touche )
{
    ...
    case TP_c: // Permuter l'affichage des faces arrières
        ...
        break;
    case TP_o: // Activer une vue geosynchrone
        Etat::orbiteGeosynchrone = !Etat::orbiteGeosynchrone;
        std::cout << " Etat::orbiteGeosynchrone= " << Etat::orbiteGeosynchrone << std::endl;
    break;

    case TP_DROITE: // Augmenter la dimension de la boîte en X
        Etat::whDim.x += 0.1;
        std::cout << " Etat::whDim= " << Etat::whDim.x << " x " << Etat::whDim.y << " x " << Etat::whDim.z << std::endl;
        break;
    case TP_GAUCHE: // Diminuer la dimension de la boîte en X
        if ( Etat::whDim.x > 10.0 ) Etat::whDim.x -= 0.1;
        std::cout << " Etat::whDim= " << Etat::whDim.x << " x " << Etat::whDim.y << " x " << Etat::whDim.z << std::endl;
        break;
    ...
    case TP_BAS: // Diminuer la dimension de la boîte en Z
        if ( Etat::whDim.z > 10.0 ) Etat::whDim.z -= 0.1;
        std::cout << " Etat::whDim= " << Etat::whDim.x << " x " << Etat::whDim.y << " x " << Etat::whDim.z << std::endl;
        break;
    ...
    case TP_0: // Afficher le point de vue « normal » (dans notre systeme solaire)
    case TP_1: // Afficher le point de vue de la planete 1
    case TP_2: // Afficher le point de vue de la planete 2
    case TP_3: // Afficher le point de vue de la planete 3
    case TP_4: // Afficher le point de vue de la planete 4
    case TP_5: // Afficher le point de vue de la planete 5
    case TP_6: // Afficher le point de vue de la planete 6
    case TP_7: // Afficher le point de vue de la planete 7
    case TP_8: // Afficher le point de vue de la planete 8
    case TP_9: // Afficher le point de vue de la planete 9
        Etat::exoplaneteChoisie = touche - TP_0;
        camera.theta = camera.phi  = 0.0;
        std::cout << " Etat::exoplaneteChoisie=" << Etat::exoplaneteChoisie << std::endl;
        break;
...
void FenetreTP::sourisMolette( int x, int y ) // Changer la distance de la caméra
{
    ...
    else if ( camera.dist > 70.0 - Etat::whDim.y ) camera.dist = 70.0 - Etat::whDim.y;
}

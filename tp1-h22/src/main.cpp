...
void FenetreTP::clavier( TP_touche touche )
{
    ...
    case TP_i: // Réinitiliaser le point de vue et Dumbo
        camera.initVar(); dumbo->initVar();
        break;
    ...
    case TP_DROITE: // Déplacer Dumbo vers +X
        if ( dumbo->position.x + dumbo->taille < Etat::dimBoite/2 ) dumbo->position.x += 0.1;
        break;
    case TP_GAUCHE: // Déplacer Dumbo vers -X
        if ( dumbo->position.x - dumbo->taille > -Etat::dimBoite/2 ) dumbo->position.x -= 0.1;
        break;
    case TP_HAUT: // Déplacer Dumbo vers +Y
        if ( dumbo->position.y + dumbo->taille < Etat::dimBoite ) dumbo->position.y += 0.1;
        break;
    case TP_BAS: // Déplacer Dumbo vers -Y
        if ( dumbo->position.y - dumbo->taille > 0.0 ) dumbo->position.y -= 0.1;
        break;
    case TP_PAGEPREC: // Déplacer Dumbo vers +Z
        if ( dumbo->position.z + dumbo->taille < Etat::dimBoite/2 ) dumbo->position.z += 0.1;
        break;
    case TP_PAGESUIV: // Déplacer Dumbo vers -Z
        if ( dumbo->position.z - dumbo->taille > -Etat::dimBoite/2 ) dumbo->position.z -= 0.1;
        break;

    case TP_FIN:
    case TP_f: // Diminuer la taille de la tête
        if ( dumbo->taille > 0.25 ) dumbo->taille -= 0.05;
        dumbo->verifierAngles();
        std::cout << " Dumbo->taille=" << dumbo->taille << " Dumbo->angleCorps=" << dumbo->angleCorps << " Dumbo->angleRotation=" << dumbo->angleRotation << std::endl;
        break;
    case TP_DEBUT:
    ...
    case TP_VIRGULE: // Diminuer l'angle de rotation de Dumbo
        dumbo->angleCorps -= 2.0;
        if ( dumbo->angleCorps < 0.0 ) dumbo->angleCorps = 0.0;
        std::cout << " Dumbo->taille=" << dumbo->taille << " Dumbo->angleCorps=" << dumbo->angleCorps << " Dumbo->anglePatte=" << dumbo->angleRotation << std::endl;
        break;
    case TP_POINT: // Augmenter l'angle de rotation de Dumbo
        dumbo->angleCorps += 2.0;
        if ( dumbo->angleCorps > 30.0 ) dumbo->angleCorps = 30.0;
        std::cout << " Dumbo->taille=" << dumbo->taille << " Dumbo->angleCorps=" << dumbo->angleCorps << " Dumbo->anglePatte=" << dumbo->angleRotation << std::endl;
        break;

    case TP_CROCHETDROIT:
    case TP_o: // Diminuer l'angle des pattes
        dumbo->angleRotation -= 2.0;
        dumbo->verifierAngles();
        std::cout << " Dumbo->taille=" << dumbo->taille << " Dumbo->angleCorps=" << dumbo->angleCorps << " Dumbo->anglePatte=" << dumbo->angleRotation << std::endl;
        break;
